...
export default function WelcomePanel({
  ...
  const hint = (
    <p className="text-[1.75rem] leading-none text-amber-100/50 sm:text-[2rem]">{t('rotateHint')}</p>
  );
  ...
  return (
    <div className="flex w-full max-w-lg flex-col gap-1 lg:max-w-md lg:gap-0">
        ...
        {hasLit ? (
          <div className="flex flex-col gap-1.5 lg:gap-6">
            <div className="flex flex-col gap-0.5 lg:gap-2.5">
              <h2 className="text-balance text-[1.25rem] font-semibold leading-none tracking-tight text-amber-50 lg:text-[1.875rem]">
                {t('litTitle')}
              </h2>
              <p className="text-pretty text-[0.875rem] leading-none text-amber-100/75 lg:text-[1.125rem] lg:leading-tight">
                {t('litMessage')}
              </p>
            </div>
      ...
      <p className="px-1 text-center text-[1rem] leading-none text-amber-100/50 lg:hidden">
        {t('rotateHint')}
      </p>
    </div>
  );
