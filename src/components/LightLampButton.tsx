...
export default function LightLampButton({ onLocationReceived }: LightLampButtonProps) {
  ...
  return (
    <div className="flex flex-col items-stretch gap-1 lg:gap-2">
      <button
        type="button"
        onClick={handleClick}
        disabled={busy}
        className="home-cta group relative inline-flex min-h-10 w-full flex-wrap items-center justify-center gap-2 overflow-hidden rounded-full bg-gradient-to-b from-amber-300 to-amber-500 px-3 py-1 text-center font-semibold leading-none whitespace-normal text-neutral-900 shadow-[0_0_36px_-4px_rgba(245,180,90,0.75)] transition-all hover:shadow-[0_0_48px_-2px_rgba(245,180,90,0.95)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-950 disabled:cursor-not-allowed disabled:opacity-80 lg:min-h-14 lg:gap-3 lg:px-8 lg:py-4"
      >
        <LampGlyph busy={busy} />
